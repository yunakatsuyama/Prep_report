...
% network design
The network consists of three convolutional layer and one fully connected layer. The filter size was 3$\times$3 and number of filter in each convolutional layer was 32, 64 and 64. Stochastic gradient descent (SDG) was used for optimizer. As the number of dataset and the size of dataset were small. Batch learning was applied instead of mini-batch learning. 
...
% = 図を見たらわかることを述べる
Fig. \ref{fig:cnn_cm} shows the recall of train and test data after 200 times of training (epoch).  
\subsection{Result1: Hand written digits classification}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\columnwidth]{cnn_cm.jpg}
